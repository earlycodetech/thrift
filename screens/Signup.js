...
import { authentication, db } from "../firebase/firebase.settings"; //this
...
const formRules = yup.object({
    ...
    gender: yup.string("invalid characters").required("This is a compulsory field"),
                        ...
                        createUserWithEmailAndPassword(authentication, values.email, values.password)
                            .then(() => {
                                ...
                                onAuthStateChanged(authentication, user => {
                                    user.uid // to get the user uid

                                    // Adding user details to Cloud Firestore
                        ...
                        return (
                            <View style={styles.form}>
                                ...
                                <SelectList
                                    ...
                                    data={gender}
                                    dropdownTextStyles={{ fontSize: 18, color: '#3C4048', }}
                                    inputStyles={{ fontSize: 24, color: '#3C4048', }}
                                    boxStyles={styles.dropDown}
                                    dropdownStyles={{ borderColor: Theme.colors.purple500 }}
                                />
                                <Text style={{ display: touched.gender && errors.gender ? 'flex' : 'none', color: 'red' }}>
